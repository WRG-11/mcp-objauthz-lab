...
**nine intentionally planted** object-level authorization flaws (scenarios
S1-S9, each an independent tool or resource handler) plus a hunt-checklist. All
...
All nine planted authorization bugs are the **point** of the exercise (see the
README hunt-checklist and the `LAB_MODE` / `LAB_S1`-`LAB_S9` toggles). They are
not vulnerabilities. Note that every toggle **defaults to `vuln`**, so a server
started with no environment set has all nine live — that is intended.
