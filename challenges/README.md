...
Seven independent BOLA scenarios, each runnable in under 5 minutes.
...
- The toggles are independent — setting `LAB_S2` never changes S1/S3/S4/S5/S6/S7.
- **But every scenario defaults to `vuln` when its variable is unset**, so
  ...
  other six to `fixed`. Run the bare `LAB_S2=vuln node src/server.js` and all
  seven bugs are live at once: as an ordinary user you can reach another org's
  data through seven different tools, which makes S1 ("find the one tool")
  unanswerable and turns the other six into guess-which-one-was-meant.
