...
Eight independent BOLA scenarios, each runnable in under 5 minutes.
...
- The toggles are independent — setting `LAB_S2` never changes S1/S3/S4/S5/S6/S7/S8.
- **But every scenario defaults to `vuln` when its variable is unset**, so
  ...
  other seven to `fixed`. Run the bare `LAB_S2=vuln node src/server.js` and all
  eight bugs are live at once: as an ordinary user you can reach another org's
  data through eight different tools/resources, which makes S1 ("find the one
